...
class Simulation():
    ...
    def simulateMPC(self):
        ...
        self.expectedDamages = []
        self.finalDamageMap = numpy.empty(patches)
        assignments = []
        fires = []

        region = self.model.getRegion()
        regionSize = region.getX().size
        samplePaths = self.model.getMCPaths()
        ...
        for ii in range(samplePaths):
            damage = 0
            assignmentsPath = copy.copy(assignments)
            ...
            for tt in range(timeSteps):
                expDamageExist = {
                        ...
                        self.expectedDamageExisting(
                                firesPath[jj], FFDI, config[kk], tt)
                        for jj in range(len(firesPath))
                        for kk in range(len(config))}

                expDamagePoten = {
                        (jj, kk):
                        self.expectedDamagePotential(jj, FFDI, config[kk], tt)
                        for jj in range(len(patches))
                        for kk in range(len(config))}

                assignmentsPath = self.assignAircraft(
                        assignmentsPath, expDamageExist, expDamagePoten)

                damage = damage + self.simulateSinglePeriod(
                        assignmentsPath, firesPath, FFDI)

                wg.computeWeather(
                        rain, precipitation, temperatureMin, temperatureMax,
                        windRegimes, windNS, windEW, FFDI, ii)

            expectedDamages.append(damage)

    def expectedDamageExisting(self, fire, ffdi, config, time):
        damage = 0
        fireTemp = copy.copy(fire)

        for tt in self.model.getLookahead():
            ffdi = self.computeNextFFDI(patch, region, time)
            damage = damage + self.computeIncrementalDamage(
                    fireTemp, region, ffdi, config, time + tt)

        return damage

    def expectedDamagePotential(self, patchID, config, time):
        damage = 0
